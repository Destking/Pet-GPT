...
class DesktopPet(QWidget):
    ...
    def init_ui(self):
        ...
        # chat with ChatGLM
        chat_action_ChatGLM = QAction("聊天_ChatGLM", self, triggered=self.show_chat_dialog_ChatGLM)

        change_icon_action = QAction("更换图标", self, triggered=self.change_icon)

        exit_action = QAction("退出", self, triggered=self.close)
        self.menu.addActions([chat_action, chat_action_ChatGLM, change_icon_action, exit_action])

        change_nickname_action = QAction("改昵称", self, triggered=self.change_nickname)
        self.menu.addActions([chat_action, chat_action_ChatGLM, change_icon_action, change_nickname_action, exit_action])

        settings_action = QAction("设置", self, triggered=self.show_settings_dialog)
        self.menu.addActions([chat_action, chat_action_ChatGLM, change_icon_action, change_nickname_action, settings_action, exit_action])
...
if __name__ == '__main__':
    ...
    if os.path.isfile('private_config.ini'):
        config = configparser.ConfigParser()
